...
Pour ce projet, j'ai choisi la démarche suivante :
 - Premierement le developpement d'une carte en static
 - Ensuite le developpement des colonnes et de la Board en utilisant les cartes
 - Puis l'implementation des props et des emit qui permettent le transfert d'information d'une composante a une autre
 - Le formulaire qui permet d'ajouter des cartes + l'edition des cartes deja presentes
 - L'ajout des v-if et computed pour l'affichage ainsi que les statistiques et la recherche filtrée
 - Enfin le localStorage avant de finir les differents bonus et de finir le designe de tout le site (sachant que ce point je l'ai un peu fait au fur et a mesure du travail)
